...
import { fetchSportSubCatTabTitle, addSportSubCatTabTitle, sportCatTitleChange } from '../../actions/sportsAction';
import { Container, Button, Form, Table, Icon, Menu, Message } from 'semantic-ui-react';
import { Link } from 'react-router-dom';

class SportSubCatTabTitle extends Component {
    componentWillMount() {
        this.props.fetchSportSubCatTabTitle(this.props.location.item.id)
    }

    onTitleChanged    = (e, { name, value }) => this.props.sportCatTitleChange(value)
    onAddSportSubCatTabTitle = () => this.props.addSportSubCatTabTitle(this.props.sportCatTitleValue, this.props.location.item.id)

    render() {
        const { sportSubCatTabTitleValues, sportWarningStatus, sportSpinnerStatus, sportCatTitleValue } = this.props;
        return (
            <Container>
                {
                ...
                <Form style={{ marginTop: 20, marginBottom: 20 }} loading={sportSpinnerStatus}>
                    ...
                    <Button onClick={this.onAddSportSubCatTabTitle}>Ekle</Button>
                </Form>
                <Table celled selectable inverted>
                    ...
                    <Table.Body>
                    {
                        sportSubCatTabTitleValues.map((item, index) => {
                            return(
                                ...
                                <Table.Cell collapsing>
                                    ...
                                    <Button as={Link} to={{
                                    pathname: "/sports/sub-cats/tab-content",
                                    item: item
...
const mapStateToProps = state => {
    const { sportSubCatTabTitleValues, sportWarningStatus, sportSpinnerStatus, sportCatTitleValue } = state.SportReducer
    return {
...
export default connect(
    mapStateToProps,
    {
        fetchSportSubCatTabTitle,
        addSportSubCatTabTitle,
        sportCatTitleChange
    }
)(SportSubCatTabTitle)